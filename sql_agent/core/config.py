...
class Settings:
    def __init__(self):
        ...
        self.enable_self_correction = os_module.getenv("ENABLE_SELF_CORRECTION", "true").lower() == "true"
        ...
        self.storage_backend = os_module.getenv("STORAGE_BACKEND", "sql_agent.storage.db.MongoStorage")
        self.vector_backend = os_module.getenv("VECTOR_BACKEND", "sql_agent.storage.vector.ChromaVectorStore")
        self.context_store = os_module.getenv("CONTEXT_STORE", "sql_agent.context.base.DefaultContextStore")
...
class System(Component):
    ...
    def instance(self, base_type):
        ...
        if base_type not in self._instances:
            ...
            if env_key:
                custom_fqn = getattr(self.settings, env_key.lower(), None) or _DEFAULT_IMPLS.get(type_fqn)
                if custom_fqn:
